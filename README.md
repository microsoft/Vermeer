...
Given the first three reference channels and a protein's ESM-C embedding, the model generates the corresponding fluorescent protein channel. `tutorial.ipynb` demonstrates this virtual staining task. To run this notebook, first download example the model checkpoints below (specifically the tokenizer and vermeer-L). The provided yaml file `vermeer.yaml` can be used to create a conda/mamba environment to run this notebook.
